...
# save_path = "DB/Vox2_Face/图片"
for root, dirs, files in os.walk(r"DB/Vox2_Face/dev_aaa/mp4/"):   # 这里就填文件夹目录就可以了
    for file in files:
        # 获取文件路径
        if ('.mp4' in file):
            path = os.path.join(root, file)   # 如：DB/Vox2_Face/id10001\1zcIwhmdeo4\00002.mp4
            # 获得视频的格式
            ...
            while (True):
                # 读帧
                ret, image = video.read()  # 其中ret是布尔值，如果读取帧是正确的则返回True，如果文件读取到结尾，它的返回值就为False。frame就是每一帧的图像，是个三维矩阵。
                ...
                if current_frame % cut_frame == 0:
                    if len(str(current_frame)) == 2:   # 针对current_frame为两位数、三位数长度不一致从而导致文件名长度不一的问题
                        cv2.imencode('.jpg',image)[1].tofile(path[:-10] + '/' + file[2:-4] + '_0' + str(current_frame) + '.jpg')
                        print('正在保存' + path[:-10] + '/' + file[2:-4] + '_0' + str(current_frame) + '.jpg')
                    if len(str(current_frame)) == 3:   
                        cv2.imencode('.jpg',image)[1].tofile(path[:-10] + '/' + file[2:-4] + '_' + str(current_frame) + '.jpg')
                        print('正在保存' + path[:-10] + '/' + file[2:-4] + '_' + str(current_frame) + '.jpg')

                    # cv2.imwrite(save_path + '/' + file[:-4] + str(current_frame) + '.jpg',image)
                    #   # file[:-4]是去掉了".mp4"后缀名，这里我的命名格式是，视频文件名+当前帧数+.jpg，使用imwrite就不能有中文路径和中文文件名

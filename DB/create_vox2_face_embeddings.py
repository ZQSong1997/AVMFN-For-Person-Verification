...
### 获得face_embedding
def get_face_embedding(model_path, names_list, filePath_list):
    # 转换颜色空间RGB or BGR
    colorSpace = "RGB"
    # 初始化mtcnn人脸检测
    ...
    image_name_list = [] # 保存人脸的name，与embeddings一一对应
    image_path_list = [] # 保存图像的路径,本工作会将其作为图片的label
    print("开始处理图片...")
    for  image_path in filePath_list:
        print("processing image：{}".format(image_path))    
        # processing image：DB/Vox2_Face/vox2_test_mp4/mp4/id00017/01dfn2spqyE/001_0050.jpg
        ...
        # 保存embeddings、label_list、 image_path_list
...
def create_face_embedding(model_path, dataset_path, out_face_emb_path, out_face_label_path):
    ...
    embeddings, image_path_list = get_face_embedding(model_path, names_list, filePath_list)
...
if __name__ == '__main__':
    model_path = 'DB/models/20180408-102900/'           # faceNet模型路径；保存有FaceNet的Pretrained Model
    in_dev_dataset_path = 'DB/Vox2_Face/vox2_dev_mp4/mp4'       # 人脸数据库路径，每一类单独一个文件夹
    out_dev_face_emb_path = 'DB/Vox2_Face/emb/vox2_face_D_embedding.npy'  # 输出embeddings的保存路径；.npy是原始数据集经Embedding后的输出结果
    out_dev_face_label_path = 'DB/Vox2_Face/emb/vox2_face_D_label.txt'  # 输出与embeddings一一对应的path标签
    
    in_test_dataset_path = 'DB/Vox2_Face/vox2_test_mp4/mp4'       
    out_test_face_emb_path = 'DB/Vox2_Face/emb/vox2_face_T_embedding.npy'  
    out_test_face_label_path = 'DB/Vox2_Face/emb/vox2_face_T_label.txt'  
    # 计算程序的处理时间
    start_date = datetime.datetime.now()
    print("开始处理时间为：{}".format(str(start_date)))
    ## 创建人脸embedding
    # create_face_embedding(model_path, in_dev_dataset_path, out_dev_face_emb_path, out_dev_face_label_path)
    create_face_embedding(model_path, in_test_dataset_path, out_test_face_emb_path, out_test_face_label_path)

...
#   先判断是否支持多卡训练
    if bool(args.mg):   # args.mg 应该指是否有多张GPU？ 为True则可以进行多卡并行训练，默认为True
        model_1gpu = Feature_Fusion(args.model)  # 先实例化model，args.model 是一个model内参字典
...
##  【2】、optimizer设置
    if args.optimizer.lower() == 'adam':    # lower()将字符串中的所有大写字母转换为小写字母
        optimizer = torch.optim.Adam(itertools.chain( model.parameters(),
            Arcface_net.parameters(), AAMsoftmax_net.parameters(), AMsoftmax_net.parameters(),
            Centerloss_net.parameters(), AngularProto_net.parameters() ),
